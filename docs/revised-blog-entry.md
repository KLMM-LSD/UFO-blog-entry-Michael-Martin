...
The three targets we decided to expose was our frontend, backend as well as Grafana. This can be scraped via a service-discovery which you can configure.  We looked at metrics such as response time, amount of requests, uptime, HTTP-500 counter etc. This is very customizable and allows you to track almost  anything in your system.
...
Some graphs has more value than others, but by doing this we have a great way of monitoring the most critical aspects of the programs non-functional parts. For instance, we have the most important one called latest id, which is monitoring all the input data that is being fed into our program and how many there are atm. We also have a graph for uptime in the frontpage to make sure it’s always running at high frequency.
...
![Alert Test](https://raw.githubusercontent.com/KLMM-LSD/UFO-blog-entry-Michael-Martin/master/Resources/Grafana-slert-test.png)
...
2. https://www.atlassian.com/devops - An in depth description of what Devops is and the benefits of this method. 
...
8. https://prometheus.io/docs/prometheus/latest/querying/basics/ - Basic PromQL description - getting started with PromQL 
